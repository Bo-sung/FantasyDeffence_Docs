...
게임 시스템 구현에 필요한 설정(Config) 테이블과 유저 런타임(User Data) 테이블을 정의합니다. 밸런싱 조정은 Config 테이블 데이터만 변경하여 코드 수정 없이 가능합니다.
...
```
config_hero ─────────┬──── config_hero_skill ──── config_skill
                     │                                  │
                     │                            config_skill_level
                     │
              config_hero_promotion
                     │
user_hero ───────────┤
  │                  │
  ├── user_hero_skill
  ├── user_deck
  ├── user_equipment (장비 시스템)
  └── building_hero_assign (영지 시스템)

config_enemy ──── config_enemy_skill ──── config_skill
     │
config_stage ──── config_wave ──── config_wave_spawn
     │
user_stage_progress

config_equipment ──── config_equipment_enhance
     │                     │
     │               config_craft_recipe ──── config_craft_material
     │
user_equipment

config_building (영지) ──── config_building_level
     │
building_placement ──── building_production
                   └──── building_hero_assign
```
...
- [영지 시스템_상세](영지%20시스템_상세.md) - 영지 Config/Runtime 테이블
